...
| F-10 | Commission send-to-payroll — `CommissionPayoutService` executes a plan's `payout` artifact for a given cycle, extracts `employee_id` + commission amount columns, validates employees, and creates Payout records with `group_id = plan.name` and `date = cycle.end_date`. Flexible column detection (supports `employee_id`/`emp_id`/`payee_id` and `commission_amount`/`commission`/`amount`/etc.). Endpoint: `POST /plans/:id/send-to-payroll`. | DONE |
